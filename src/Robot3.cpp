...
bool Robot3 :: isSafeItem(int x,int y,bool **visited)
{
    if(x<0 || y<0 || x>=n || y>=m)
        return 0;
    if(visited[x][y]==1)
...
bool Robot3 :: isSafe(int x,int y,bool **visited)
{
    if(x<0 || y<0 || x>=n || y>=m)
        return 0;
...
void Robot3 :: dfs(int x,int y,int nr)
{
...
void Robot3 :: Prelucrare(int **x1,int **y1,int x,int y)
{
    int nr=1,xs=x,ys=y;
    int **zzz;
    zzz=new int *[n];
