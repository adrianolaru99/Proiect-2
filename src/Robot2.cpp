...
bool Robot2 :: isSafeItem(int x,int y,bool **visited)
{
    if(x<0 || y<0 || x>=n || y>=m)
        return 0;
    if(visited[x][y]==1)
...
bool Robot2 :: isSafe(int x,int y,bool **visited)
{
    if(x<0 || y<0 || x>=n || y>=m)
        return 0;
...
void Robot2 :: dfs(int x,int y,int nr)
{
...
void Robot2 :: Prelucrare(int **x1,int **y1,int x,int y)
{
    int nr=1,xs=x,ys=y;
    int **zzz;
    zzz=new int *[n];
