...
bool Robot1 :: isSafeItem(int x,int y,bool **visited)
{
    if(x<0 || y<0 || x>=n || y>=m)
        return 0;
    if(visited[x][y]==1)
...
bool Robot1 :: isSafe(int x,int y,bool **visited)
{
    if(x<0 || y<0 || x>=n || y>=m)
        return 0;
...
void Robot1 :: dfs(int x,int y,int nr)
{
...
void Robot1 :: Prelucrare(int **x1,int **y1,int x,int y)
{
    int nr=1,xs=x,ys=y;
    int **zzz;
    zzz=new int *[n];
